    ...
    <FormProvider {...form}>
      <form id="edit-measure" onSubmit={form.handleSubmit(handleSubmit)}>
        <div className="grid grid-cols-1 gap-2 mb-2">
          <FormFieldInput type="text" label="Type of measure" name="name" className="pl-10" Icon={Ruler} />
        </div>
        <DialogFooter disabled={!form.formState.isDirty || isPending} isLoading={isPending} label="Submit changes" formName="edit-measure" className="mt-6" />
      </form>
    </FormProvider>
  );
};
