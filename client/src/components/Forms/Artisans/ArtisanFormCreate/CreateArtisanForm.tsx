  ...
  return (
    <FormProvider {...form}>
      <form id="artisan-form" onSubmit={form.handleSubmit(handleSubmit)}>
        ...
        <Separator className="mt-4 mb-2" />
        <FormTextareaInput name="note" label="Artisan note" type="text" placeholder="Artisan notes..." />
        <DialogFooter disabled={!form.formState.isDirty || isPending} label="Submit" formName="artisan-form" className="mt-6" />
      </form>
    </FormProvider>
  );
};
