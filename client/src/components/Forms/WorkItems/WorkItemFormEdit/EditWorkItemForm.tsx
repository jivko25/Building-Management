    ...
    <FormProvider {...form}>
      <form id="work-item-edit" onSubmit={form.handleSubmit(handleSubmit)}>
        <div className="grid grid-cols-1 gap-2 mb-2">
          <FormFieldInput name="name" label="Work item name" type="text" className="pl-10" Icon={ClipboardList} />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 content-around gap-2 pt-1">
          <FormDatePicker name="start_date" label="Select a start date" selected={new Date(`${workItem && workItem.start_date}`).toLocaleDateString().slice(0, 10)} />
          <FormDatePicker name="end_date" label="Select an end date" selected={new Date(`${workItem && workItem.end_date}`).toLocaleDateString().slice(0, 10)} />
        </div>
        <Separator className="mt-2 " />
        <div className="grid grid-cols-1 sm:grid-cols-2 content-around gap-2 pt-1">
          <TaskItemStatusSelector name="status" label="Select status" defaultVal={workItem?.status} />
        </div>
        <Separator className="mt-4 mb-2" />
        <FormTextareaInput name="note" label="Work item note" type="text" placeholder="Work item notes..." />
        <DialogFooter disabled={!form.formState.isDirty || isPending} label="Submit changes" formName="work-item-edit" className="mt-6" />
      </form>
    </FormProvider>
  );
};
